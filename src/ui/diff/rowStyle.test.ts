...
describe("difftastic cell styling", () => {
  ...
  test("novelty foregrounds clear the readability floor on every built-in theme", () => {
    for (const base of THEMES) {
      for (const theme of [base, withTransparentSurfaces(base)]) {
        for (const kind of ["addition", "deletion"] as const) {
          const fg = difftasticNoveltyFg(kind, theme);
          const background =
            theme.contextBg === TRANSPARENT_BACKGROUND
              ? theme.appearance === "dark"
                ? "#000000"
                : "#ffffff"
              : theme.contextBg;
          expect(contrastRatio(fg, background)).toBeGreaterThanOrEqual(3.1);
        }
      }
    }
  });
