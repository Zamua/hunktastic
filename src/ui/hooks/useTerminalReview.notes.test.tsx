...
import { captureReviewNoteQuote } from "../../core/review/noteResolution";
...
function useTempNoteStore() {
  ...
  const stateDir = createTempDir("hunkt-notes-state-");
...
/** Author one persisted record the way a live session would, quoting the file's content. */
function storedNoteFor(
  ...
  return {
    ...
    ...captureReviewNoteQuote(projectReviewDocument([file]).files[0]!, "new", line),
  };
}
...
async function renderNotesController(
  files: DiffFile[],
  noteScope?: NoteScope,
...
/** Read the store's mutable notes as (id, resolution) pairs, live before user. */
function noteResolutions(controller: TerminalReview) {
  const snapshot = controller.store.getSnapshot();
  return [...snapshot.liveNotes, ...snapshot.userNotes].map(
    (entry) => [entry.note.id, entry.resolution] as const,
  );
}

describe("useTerminalReview note persistence", () => {
  test("places stored notes back inline on the row they were written against", async () => {
    ...
    try {
      ...
      expect(controller.userNotesByFileId["alpha.ts"]).toMatchObject([
        { id: "user:1", line: 10, hunkIndex: 0, side: "new" },
      ]);
      expect(controller.liveCommentsByFileId["alpha.ts"]).toMatchObject([
        { id: "mcp:1", line: 12, hunkIndex: 0, side: "new" },
      ]);
      expect(noteResolutions(controller)).toEqual([
        ["mcp:1", "active"],
        ["user:1", "active"],
      ]);
    } finally {
  ...
  test("a note whose line was edited away stays visible at its coordinate and reports stale", async () => {
    useTempNoteStore();
    ...
    try {
      const controller = expectValue(controllerRef.current);
      // The stale-note visibility policy: last known anchor, not gone entirely.
      expect(controller.userNotesByFileId["alpha.ts"]).toMatchObject([
        { id: "user:1", line: 10, hunkIndex: 0 },
      ]);
      expect(noteResolutions(controller)).toEqual([["user:1", "stale"]]);
    } finally {
      await destroy();
    }
  });

  test("a reload re-anchors restored notes through document reconciliation", async () => {
    useTempNoteStore();
    const authored = fixtureFile(10, "const line10 = 100;");
    const scope: NoteScope = { worktreeRoot: "/repo", reviewTarget: "diff" };
    ...
    try {
      expect(noteResolutions(expectValue(controllerRef.current))).toEqual([["mcp:1", "active"]]);

      // The noted line moves down three lines: still active, on its new line.
      const shifted = [...BASE_LINES];
      shifted[9] = "const line10 = 100;";
      await reload({
        files: [
          createTestDiffFile({
            after: lines("const i1 = 1;", "const i2 = 2;", "const i3 = 3;", ...shifted),
            before: lines(...BASE_LINES),
            context: 3,
            id: "alpha.ts",
            path: "alpha.ts",
      ...
      expect(noteResolutions(controller)).toEqual([["mcp:1", "active"]]);
      expect(controller.liveCommentsByFileId["alpha.ts"]).toMatchObject([
        { id: "mcp:1", line: 13 },
      ]);

      // The noted text is edited away: stale, still shown at the stored coordinate.
      await reload({
        files: [fixtureFile(10, "const line10 = 999;")],
        noteScope: scope,
      });
      controller = expectValue(controllerRef.current);
      expect(noteResolutions(controller)).toEqual([["mcp:1", "stale"]]);
      expect(controller.liveCommentsByFileId["alpha.ts"]).toMatchObject([
        { id: "mcp:1", line: 10 },
      ]);
    } finally {
  ...
  test("notes written under one worktree never surface under another", async () => {
    ...
    try {
      expect(noteResolutions(expectValue(controllerRef.current))).toEqual([]);
  ...
  test("a review without a scope writes nothing at all", async () => {
  ...
  test("an added note is persisted and comes back on the next review of the same scope", async () => {
    ...
    try {
      const controller = expectValue(second.controllerRef.current);
      expect(controller.liveCommentsByFileId["alpha.ts"]).toMatchObject([
        { id: "mcp:1", line: 10, summary: "Agent found this" },
      ]);
      expect(noteResolutions(controller)).toEqual([["mcp:1", "active"]]);
    } finally {
...
describe("note persistence through the real loader path", () => {
  test("a note added to a working-tree review returns to the next load of that worktree", async () => {
    useTempNoteStore();
    const repo = createTempDir("hunkt-notes-repo-");
    ...
    try {
      const controller = expectValue(second.controllerRef.current);
      expect(noteResolutions(controller)).toEqual([["mcp:1", "active"]]);
      // The loader assigns runtime file ids, so the inline map is keyed by that id.
      const fileId = expectValue(
        controller.allFiles.find((candidate) => candidate.path === "alpha.ts"),
      ).id;
      expect(controller.liveCommentsByFileId[fileId]).toMatchObject([
        { id: "mcp:1", line: 10, summary: "Reviewed here" },
      ]);
    } finally {
      await second.destroy();
    }
