...
describe("parseCli", () => {
  ...
  test("uses --fast alone as the shortest fast working-tree review", async () => {
    expect(await parseCli(["bun", "hunkt", "--fast"])).toMatchObject({
      kind: "vcs",
      staged: false,
      options: { fast: true },
    });
  ...
  test("accepts --fast before or after a review command", async () => {
    const prefixed = await parseCli(["bun", "hunkt", "--fast", "diff"]);
    const commandOption = await parseCli(["bun", "hunkt", "diff", "--fast"]);
  ...
  test("routes shorthand diff options through the diff parser", async () => {
    expect(await parseCli(["bun", "hunkt", "--fast", "--staged"])).toMatchObject({
      kind: "vcs",
      staged: true,
      options: { fast: true },
    });
    expect(await parseCli(["bun", "hunkt", "--fast", "--watch"])).toMatchObject({
      kind: "vcs",
      options: { fast: true, watch: true },
    });
    expect(await parseCli(["bun", "hunkt", "--fast", "--theme", "nord"])).toMatchObject({
      kind: "vcs",
  ...
  test("routes shorthand targets and direct file pairs through the diff parser", async () => {
    expect(await parseCli(["bun", "hunkt", "--fast", "HEAD"])).toMatchObject({
      kind: "vcs",
      range: "HEAD",
      options: { fast: true },
    });
    ...
    expect(await parseCli(["bun", "hunkt", "--fast", left, right])).toMatchObject({
      kind: "diff",
      left,
      right,
      options: { fast: true },
  ...
  test("keeps top-level help and version handling ahead of --fast shorthand", async () => {
    const help = await parseCli(["bun", "hunkt", "--fast", "--help"]);
    const version = await parseCli(["bun", "hunkt", "--fast", "--version"]);
  ...
  test("accepts leading --fast and --experimental in either order", async () => {
    for (const flags of [
      ...
      expect(await parseCli(["bun", "hunkt", ...flags, "show"])).toMatchObject({
        kind: "show",
  ...
  test("keeps fast disabled by default and treats it as a pathspec after --", async () => {
    const normal = await parseCli(["bun", "hunkt", "diff"]);
    const pathspec = await parseCli(["bun", "hunkt", "diff", "--", "--fast"]);
  ...
  test("parses sidebar toggles", async () => {
    const shown = await parseCli(["bun", "hunkt", "diff", "--sidebar"]);
    const hidden = await parseCli(["bun", "hunkt", "diff", "--no-sidebar"]);
    const unset = await parseCli(["bun", "hunkt", "diff"]);
  ...
  test("keeps paired-flag-shaped pathspecs after the option separator", async () => {
    ...
    for (const [pathspec, option] of cases) {
      const parsed = await parseCli(["bun", "hunkt", "diff", "--", pathspec]);
  ...
  test("prints the bundled skill path for hunkt skill path", async () => {
    ...
    expect(parsed.text).toEndWith(`${join("skills", "hunkt-review", "SKILL.md")}\n`);
  });

  test("prints a named bundled skill path, by name or alias", async () => {
    for (const requested of ["hunkt-extensions", "extensions"]) {
      ...
      expect(parsed.text).toEndWith(`${join("skills", "hunkt-extensions", "SKILL.md")}\n`);
    }
  });
  ...
  test("parses session highlight clear globally and per file", async () => {
    expect(await parseCli(["bun", "hunkt", "session", "highlight", "clear", "session-1"])).toEqual({
      kind: "session",
  ...
  test("rejects unknown session highlight subcommands", async () => {
    await expect(parseCli(["bun", "hunkt", "session", "highlight", "paint"])).rejects.toThrow(
...
describe("parseCli argument validation", () => {
  ...
  test("rejects prefixed review flags for non-review commands", async () => {
    ...
    await expect(parseCli(["bun", "hunkt", "--fast", "session", "list"])).rejects.toThrow(
  ...
  test("rejects unknown top-level, skill, daemon, stash, and comment subcommands", async () => {
    ...
    await expect(parseCli(["bun", "hunkt", "skill", "path", "bogus"])).rejects.toThrow(
      'Unknown skill "bogus". Bundled skills are hunkt-review and hunkt-extensions.',
    );
    ...
    await expect(
      parseCli(["bun", "hunkt", "skill", "path", "hunkt-review", "extra"]),
