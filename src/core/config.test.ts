...
describe("engine configuration", () => {
  ...
  test("layers engine through user config, repo config, HUNKT_ENGINE, and the CLI flag", () => {
    const home = createTempDir("hunk-config-home-");
    const repo = createTempDir("hunk-config-repo-");
    createRepo(repo);
    mkdirSync(join(home, ".config", "hunkt"), { recursive: true });
    ...
    const fromEnv = resolveConfiguredCliInput(createPatchPagerInput(), {
      cwd: repo,
      env: { HOME: home, HUNKT_ENGINE: "difftastic" },
    });
  ...
  test("ignores an invalid HUNKT_ENGINE value with a startup notice", () => {
    const home = createTempDir("hunk-config-home-");
    const repo = createTempDir("hunk-config-repo-");
    createRepo(repo);
    ...
    const resolved = resolveConfiguredCliInput(createPatchPagerInput(), {
      cwd: repo,
      env: { HOME: home, HUNKT_ENGINE: "sparkles" },
    });

    expect(resolved.input.options.engine).toBe("difftastic");
    expect(resolved.startupNotices?.map((notice) => notice.message)).toEqual([
      'Ignored HUNKT_ENGINE="sparkles". Valid engines: pierre, difftastic.',
    ]);
  });
  ...
  test("honors difft_path from user config and lets HUNKT_DIFFT_PATH outrank it", () => {
    const home = createTempDir("hunk-config-home-");
    const repo = createTempDir("hunk-config-repo-");
    createRepo(repo);
    mkdirSync(join(home, ".config", "hunkt"), { recursive: true });
    ...
    const fromEnv = resolveConfiguredCliInput(createPatchPagerInput(), {
      cwd: repo,
      env: { HOME: home, HUNKT_DIFFT_PATH: "/nix/difft" },
    });
