...
import { reviewStreamCursors } from "../../review/navigation";
...
describe("difftastic engine through the two-file loader", () => {
  test.skipIf(isWindows)(
    ...
    async () => {
      ...
      // Navigation invariant: mapped hunks project into the review document and
      // every cursor indexes a real hunk in its file.
      const document = projectReviewDocument(bootstrap.changeset.files);
      const cursors = reviewStreamCursors(
        document.files.map((entry) => ({ fileKey: entry.key, hunkCount: entry.hunks.length })),
      );
      expect(cursors.length).toBeGreaterThan(0);
      for (const cursor of cursors) {
        const target = document.files.find((entry) => entry.key === cursor.fileKey);
        expect(target).toBeDefined();
        expect(cursor.hunkIndex).toBeGreaterThanOrEqual(0);
        expect(cursor.hunkIndex).toBeLessThan(target!.hunks.length);
      }
