...
`[keybindings]` is read from your user config only, never from a repository's `.hunkt/config.toml`: which keys do what is a property of your keyboard and habits, so a checkout you review cannot rearrange them.
