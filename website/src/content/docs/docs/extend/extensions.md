...
Writing one with a coding agent? `hunkt skill path hunkt-extensions` prints a bundled skill that maps these touchpoints for agents, the way `hunkt skill path` does for reviewing.
