...
Writing one with a coding agent? `hunkt skill path hunkt-extensions` prints a
...
> **Identity note:** `HUNK_EXTENSION_API_VERSION`, `HUNK_VENDOR_EXTENSION_ID`,
> and the `"hunk"` package.json manifest field keep the `HUNK`/`hunk` spelling
> on purpose. They are the published third-party extension contract; renaming
> them would break every existing extension for no gain. Do not sweep them to
> `HUNKT`/`hunkt` in an identity pass. The same holds for internal wire tokens
> (the `hunk-daemon` broker client id, the `hunk-review-capability` header, the
> `hunk-review-heartbeat` SSE frame, `hunk.`-namespaced command ids, and the
> `HUNK_*` protocol constants): no user types them, and both ends of each wire
> ship in this repo, so a rename buys nothing and risks a missed pair.
