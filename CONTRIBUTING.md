...
- If you want temporary local review notes, you can use `.hunkt/latest.json`, but do not commit it.
- `hunkt diff` includes untracked working-tree files by default. Use `--exclude-untracked` if you want to review tracked changes only.

## Release notes

- The npm package name is `hunkdiff`.
- The installed CLI command remains `hunkt`.
